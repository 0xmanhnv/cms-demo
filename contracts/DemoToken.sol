...
import "./interfaces/IBotPrevent.sol"; // from wanakafarm

contract DemoToken is ERC20PresetMinterPauser {
    
    IBotPrevent public BP;
    bool public bpEnabled = false;
    uint256 public constant MAX_SUPPLY = 10 * (10 ** 8) * (10 ** 18); // 1.000.000.000 TDV
    
    
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "TDV: ADMIN role required");
        _;
    }

    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, _msgSender()), "TDV: MINTER role required");
        _;
    }
    
    constructor() ERC20PresetMinterPauser("Demo token", "DDT") {
    ...
    function mint(address to, uint256 amount) public virtual override onlyMinter {
        require(totalSupply() + amount <= MAX_SUPPLY, "TDV: Max supply exceeded");
        _mint(to, amount);
    }
